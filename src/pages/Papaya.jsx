  ...
  return (
    <div className="p-6 bg-orange-500 min-h-screen">
      <input
        ...
        className="mb-6 p-3 border border-orange-300 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-orange-300"
      />

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6">
        {filteredPapayas.length > 0 ? (
          filteredPapayas.map((fruit) => (
            <FruitCard key={fruit.id} fruit={fruit} />
          ))
        ) : (
          <p className="text-center text-gray-500 mt-4">
            No papaya variety found.
          </p>
        )}
      </div>
    </div>
